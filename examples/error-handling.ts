...
async function inputValidationExample() {
  ...
  function validateInput(text: string, options: RenderOptions = {}): string[] {
    ...
    if (options.palette) {
      if (Array.isArray(options.palette)) {
        if (options.palette.length === 0) {
          ...
          options.palette.some((color) => !color.match(/^#[0-9a-fA-F]{6}$/))
        ) {
        ...
        if (!availablePalettes.includes(options.palette)) {
          errors.push(
            `Palette "${options.palette}" is not available. Available: ${availablePalettes.join(', ')}`
          );
        }
      }
    }
    ...
    if (
      options.direction &&
      !['vertical', 'horizontal', 'diagonal'].includes(options.direction)
    ) {
  ...
  for (const input of testInputs) {
    console.log(
      `Validating: text="${input.text}", options=${JSON.stringify(input.options)}`
    );
...
async function defensiveProgrammingExample() {
  ...
  class LogoGenerator {
    ...
    private sanitizeOptions(options: Partial<RenderOptions>): RenderOptions {
      ...
      if (
        ...
        console.log(
          `⚠️ Invalid direction "${options.direction}", using default`
        );
      }
  ...
  for (const testCase of testCases) {
    console.log(
      `Input: text=${testCase.text}, options=${JSON.stringify(testCase.options)}`
    );
    const result = await generator.generateLogo(
      testCase.text,
      testCase.options
    );
...
async function main() {
  ...
  console.log(
    'Learning how to handle errors gracefully and build robust applications!\n'
  );
  console.log('='.repeat(60) + '\n');
